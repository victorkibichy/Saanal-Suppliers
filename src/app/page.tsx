    ...
    <div className="flex flex-col min-h-screen bg-gray-50 text-gray-900 font-sans">
      ...
      <header className="w-full flex justify-between items-center px-8 sm:px-20 py-6 bg-white shadow-md fixed top-0 left-0 right-0 z-50">
        ...
        <nav className="flex gap-6 text-lg">
          {["About Us", "Services", "Our Offerings", "Contact Us"].map((item, index) => (
            <a key={index} href={`#${item.toLowerCase().replace(" ", "")}`} className="hover:text-blue-600 transition duration-300">
              {item}
            </a>
          ))}
        </nav>
